#!/usr/bin/env node
/**
 * promptsmith — command line for the whole thing.
 *
 *   promptsmith start          start daemon + tray host
 *   promptsmith stop           stop both
 *   promptsmith status         what is running, and is the key working
 *   promptsmith rewrite "..."  rewrite from the terminal (no hotkey involved)
 *   promptsmith build          recompile the tray host
 *   promptsmith autostart on   run at logon
 *   promptsmith history        recent rewrites
 */
...
const HOST_EXE = path.join(ROOT, "host", "bin", "PromptsmithHost.exe");
const DAEMON = path.join(ROOT, "daemon", "server.mjs");
const STATE_DIR = path.join(process.env.APPDATA || path.join(os.homedir(), ".config"), "Promptsmith");
const PID_FILE = path.join(STATE_DIR, "daemon.pid");
const LOG_FILE = path.join(STATE_DIR, "daemon.log");
const TASK_NAME = "PromptsmithLogon";
...
async function startDaemon() {
  ...
  // alive — so `promptsmith start` would print its success line and then hang
...
async function hostRunning() {
  ...
  try {
    const { stdout } = await execFileAsync("tasklist", [
      "/FI", "IMAGENAME eq PromptsmithHost.exe", "/NH", "/FO", "CSV",
    ]);
    return stdout.includes("PromptsmithHost.exe");
  } catch {
    return false;
  }
}
...
async function stopHost() {
  ...
  try {
    await execFileAsync("taskkill", ["/IM", "PromptsmithHost.exe", "/F"]);
...
async function setAutostart(enable) {
  ...
  writeFileSync(
    shim,
    [
      "' Generated by `promptsmith autostart on`. Starts Promptsmith with no",
      "' console window. 0 = hidden, False = do not wait for it to exit.",
      'Set sh = CreateObject("WScript.Shell")',
      `sh.Run """${process.execPath}"" ""${path.join(ROOT, "promptsmith.mjs")}"" start", 0, False`,
      "",
    ].join("\r\n"),
    "utf8"
  );
...
async function cmdStatus() {
  ...
  console.log(c.bold("Promptsmith"));
  if (health) {
...
function usage() {
  console.log(`${c.bold("promptsmith")} — always-on prompt rewriter
  ...
  ${c.dim("Hotkeys (change them in %APPDATA%\\Promptsmith\\config.json):")}
